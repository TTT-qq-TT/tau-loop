...
Codex 会安装 TauLoop、判断项目是新项目还是已有项目、创建必要的项目记录，并先把工作拆成可检查的部分。你不需要先知道 `spec`、`checkpoint` 或 `harness` 分别是什么。
...
不需要每次都读完这些文件。`AGENTS.md` 是入口，告诉 Codex 先读什么；它通常会先读 `.harness/memory.md`、`.harness/plan.md`，再按计划指向当前的 task spec。详细的架构背景、验证规则和历史记录只在任务确实需要时加载。
...
| 长任务约定 | `AGENTS.md` 里 `Long-Running Tasks` 章节：进程由 OS 托管，agent 周期性检查。 |
| verifier | 在阶段命令结束后判断它是否真的达标的命令。 |
| τ = 2π | 一段完整的工作：specify、run、verify、checkpoint，然后进入下一段或交给你决定。 |
